...
graph TD
    ...
    Notify["[External]<br/>Mock Notification<br/>(Hệ thống thông báo giả lập)"]:::external
...
graph TD
    ...
    subgraph "UniHub Workshop System"
        ...
        MobileApp["[Container: Android + Room]<br/>Mobile Application<br/>(App quét QR điểm danh Offline-first)"]:::container
...
graph LR
    ...
    subgraph Client ["Cụm Client"]
        ...
        Android["Mobile App Android"]:::client
    end
    ...
    Android <-->|1 Sync Offline Data| Gateway
...
- SQLite / Room (Local DB): Lưu trữ dữ liệu danh sách mã QR trên thiết bị Android của nhân sự để phục vụ luồng check-in offline.
...
- Phân quyền (Authorization): Định nghĩa 3 Role với quyền hạn thiết lập chặt chẽ tại Middleware của Backend API:
  - STUDENT: Chỉ có quyền GET /workshops, POST /registrations (áp dụng cho chính user_id đó) và GET /registrations/my-tickets.
  ...
  - CHECKIN_STAFF: Chỉ có quyền gọi API đồng bộ danh sách QR GET /sync-data và cập nhật trạng thái điểm danh PUT /registrations/checkin.
...
- Cách hoạt động: Khi API gọi sang Cổng thanh toán (Mock), nếu tỷ lệ lỗi (timeout/500) vượt ngưỡng 50% trong 10 giây, Circuit Breaker sẽ chuyển sang trạng thái OPEN.
...
- Cách hoạt động:
  ...
  - Nếu cổng thanh toán chập chờn, mạng lag, sinh viên bấm nút thanh toán thêm 3 lần nữa (cùng gửi lên key_123). Backend thấy key này ĐÃ có trong Redis, sẽ lập tức trả về kết quả “Thanh toán thành công” của lần xử lý trước đó mà không thực hiện gọi cổng thanh toán lần 2. Đảm bảo an toàn tuyệt đối tài chính.
...
- ADR 3: Thiết kế App Native Android cho luồng Check-in thay vì Web App
  - Đánh đổi: Cần code thêm một nền tảng mới (Android/Java) thay vì dùng chung ReactJS. Tuy nhiên, trình duyệt Web (kể cả PWA) đôi khi không ổn định trong việc lưu trữ offline khi bộ nhớ điện thoại đầy (bị hệ điều hành tự động dọn Cache). Ứng dụng Android kết hợp cơ sở dữ liệu cục bộ (Room SQLite) đảm bảo dữ liệu điểm danh offline được lưu trữ bền vững, an toàn và đồng bộ hóa (sync) chính xác 100% khi có mạng lại.
